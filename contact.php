...
function sh_set( $var, $key )
{
	...
	if( !count( $_POST ) ) return;
		...
		$validation->set_rules('contact_phone','<strong>'.'Phone'.'</strong>', 'min_length[5]');
		...
		if($validation->run() !== FALSE && empty($validation->_error_array))
		{
			...
			$message .= "\r\n"."Contact Email:".$validation->post('contact_email')."\r\n";
			$message .= "\r\n"."Contact Phone:".$validation->post('contact_phone')."\r\n";
			$message .= "\r\n"."Contact Subject:".sh_set( $_POST, 'contact_subject')."\r\n";
			$message .= "\r\n"."Contact Message:".$validation->post('contact_message'); 
